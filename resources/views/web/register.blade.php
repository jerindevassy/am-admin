    ...
    <div class="w-full max-w-md bg-white p-8 rounded-xl shadow-lg">
        ...
        <form action="{{ route('createUser') }}" method="POST">
            ...
            <button type="submit" style="background-color:black;"
                class="w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 rounded-lg transition duration-300">
                Register
            </button>
        </form>

        <p class="mt-4 text-center text-gray-600">Already have an account? 
            <a href="{{ route('userLogin') }}" class="text-blue-500 hover:underline">Login</a>
        </p>
    </div>
</div>
@endsection
